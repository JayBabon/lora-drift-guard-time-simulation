...
class TreeMeshFixedGt
{
public:
  TreeMeshFixedGt(double intervalS,
                  double simTimeS,
                  double hopDelayS,
  ...
  void Run()
  {
    NS_LOG_INFO("Phase 1 Step 5: 6-node tree-based mesh with fixed guard-time model");
    NS_LOG_INFO("Drift settings (ppm): src=" << m_srcPpm
                 << " relay=" << m_relayPpm
                 << " sink=" << m_sinkPpm);
    NS_LOG_INFO("Fixed GT (ms): " << m_fixedGtMs);
...
private:
  ...
  void ScheduleSourceStart(uint32_t src, Time firstGlobal)
  {
    ...
    {
      Simulator::Schedule(firstGlobal, &TreeMeshFixedGt::SourceSend, this, src);
    }
  }
  ...
  void SourceSend(uint32_t src)
  {
    ...
    Simulator::Schedule(m_hopDelay,
                        &TreeMeshFixedGt::ReceiveAtRelay,
                        this,
                        relay,
                        p);

    // Next source send based on LOCAL clock
    ...
    if (nextGlobal < m_simTime)
    {
      ...
      Simulator::Schedule(delay, &TreeMeshFixedGt::SourceSend, this, src);
    }
  }

  bool CheckWindow(std::map<uint32_t, ExpectedWindow>& table,
                   uint32_t streamId,
                   Time arrivalLocal,
                   Time& errOut)
  {
    auto& w = table[streamId];
    ...
    bool ok = std::fabs(errOut.GetSeconds()) <= m_fixedGt.GetSeconds();
    w.expectedLocal += m_intervalLocal;
    return ok;
  }
  ...
  void ReceiveAtRelay(uint32_t relay, PacketInfo p)
  {
    ...
    bool ok = CheckWindow(m_expectedAtRelay[relay], p.sourceId, nowLocal, err);
    ...
    if (ok)
    {
      ...
      Simulator::Schedule(delay,
                          &TreeMeshFixedGt::ForwardFromRelay,
                          this,
                          relay,
                          p);
    }
  ...
  void ReceiveAtSink(uint32_t sink, uint32_t fromRelay, PacketInfo p)
  {
    ...
    bool ok = CheckWindow(m_expectedAtSink, p.sourceId, nowLocal, err);
  ...
  void SaveCsv()
  {
    ...
    out << "simTimeS,intervalS,hopDelayS,relayProcDelayS,srcPpm,relayPpm,sinkPpm,fixedGtMs,"
           "tx_total,relay_rx_total,relay_rx_ok,relay_viol,relay_fwd_total,"
           "sink_rx_total,sink_rx_ok,sink_viol,pdr_ok_percent,avg_delay_ok_ms\n";
...
private:
  ...
  std::map<uint32_t, std::map<uint32_t, ExpectedWindow>> m_expectedAtRelay;
  std::map<uint32_t, ExpectedWindow> m_expectedAtSink;
...
main(int argc, char *argv[])
{
  ...
  std::string csvPath = "contrib/lora_drift_gt/results/phase1_step5_fixed_gt.csv";
  ...
  cmd.AddValue("fixedGtMs", "Fixed guard time (ms) used as +/-GT window", fixedGtMs);
