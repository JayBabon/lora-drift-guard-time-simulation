...
EVID_DIR="${NS3_DIR}/contrib/lora_drift_gt/evidence/phase2_step5"
...
RAW_CSV="${RES_DIR}/phase2_step5_raw_${STAMP}.csv"
RUN_LOG="${LOG_DIR}/phase2_step5_${STAMP}.log"
...
echo "scenario_id,seed,run,gtModel,srcPpm,relayPpm,sinkPpm,tx_total,relay_rx_total,relay_rx_ok,relay_viol,relay_fwd_total,sink_rx_total,sink_rx_ok,sink_viol,pdr_ok_percent,plr_percent,avg_delay_ok_ms,avg_gt_used_ok_ms,total_viol,violation_rate" > "${RAW_CSV}"
...
do
  ...
  for run in $(seq 1 "${K_RUNS}"); do
    TMP_CSV="$(mktemp "${RES_DIR}/tmp_phase2_step5_XXXXXX.csv")"
    ...
    tail -n +2 "${TMP_CSV}" | awk -F, -v sid="${scenario_id}" '
      BEGIN{OFS=","}
      {
        # TMP CSV fields from the ns-3 model:
        # 1 seed, 2 run, 3 gtModel, 4 srcPpm, 5 relayPpm, 6 sinkPpm,
        # 7 tx_total, 8 relay_rx_total, 9 relay_rx_ok, 10 relay_viol,
        # 11 relay_fwd_total, 12 sink_rx_total, 13 sink_rx_ok, 14 sink_viol,
        # 15 pdr_ok_percent, 16 plr_percent, 17 avg_delay_ok_ms, 18 avg_gt_used_ok_ms
        total_viol = $10 + $14;
        violation_rate = ($7 == 0 ? 0 : (100.0 * total_viol / $7));
        print sid,$0,total_viol,violation_rate
      }' >> "${RAW_CSV}"
...
{
  echo "PHASE 2 STEP 5 - FULL OBJECTIVE 2 EXECUTION PROOF"
  echo "================================================="
  date
  echo
  echo "[Latest raw CSV]"
